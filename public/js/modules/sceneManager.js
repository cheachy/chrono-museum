//Handles loading eras, scenery, and 3D models into the scene
const SceneManager = {
    ...
    for (let i = 0; i < 5; i++) {
        ...
        if(label && itemEntity && items && items[i]) {
            ...
            if (itemData.model) {
                ...
                // If no model, remove GLB attribute and show a box
  ...
  buildScenery: function(era) {
    ...
    if(era === 'prehistory') {
      let volcano = document.createElement('a-entity');
      volcano.setAttribute('gltf-model', 'assets/prehistory/volcano.glb');
      volcano.setAttribute('position', '0 0 -40'); 
      volcano.setAttribute('scale', '50 50 50');
      container.appendChild(volcano);

      this.spawnSafeModel(container, 'assets/prehistory/large_bone.glb', 1, '2.5 2.5 2.5');
    } else if (era === 'classical') {
      this.spawnSafeModel(container, './assets/classical/Column.glb', 10, '1 1 1');
      ...
      this.spawnSafeProps(container, 'cylinder', '#463016', 8, {h:1, r:0.4}, false); // Barrels
       ...
       this.spawnSafeProps(container, 'box', '#111', 10, {h:15, r:0.5}, true); // Skyscrapers
    }
  },

  spawnSafeModel: function(container, modelPath, count, scaleStr) {
    const noGoZones = [
      {x: 0, z: 0, r: 4}, {x: -4, z: -3, r: 2}, {x: -2, z: -2.5, r: 2},
      {x: 0, z: -2, r: 2}, {x: 2, z: -2.5, r: 2}, {x: 4, z: -3, r: 2}
    ];
    ...
    while(spawned < count && attempts < 100) {
      ...
      for(let zone of noGoZones) {
        ...
        if(dist < zone.r) { safe = false; break; }
      }
      ...
      if(safe) {
        ...
        el.setAttribute('scale', scaleStr || "1 1 1");
        
        // FIXED: Added backticks for rotation
        let randomRot = Math.random() * 360;
        el.setAttribute('rotation', `0 ${randomRot} 0`);
  ...
  loadBackgroundModels: function(backgroundModels) {
    ...
    if(!container) {
      console.warn("Background models container not found");
      return;
    }
  ...
  removeBackgroundModels: function() {
    ...
    console.log("Removed background models");
  }
};
